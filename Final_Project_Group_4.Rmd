---
title: "Final Project Report Group 4: The Association of Mental and Sociodemographic Factors on Sleep Quality"
...
**Sleep_well**(Binary): Sleep quality (Sleep_well = 1:sleep good; Sleep_well = 0:sleep
bad )
...
A key assumption in our analysis was the use of ordinal encoding for categorical variables. This approach involves converting category data with natural ordered relationships into numerical values, with smaller values typically indicating a lower degree. While this encoding assumes consistency in the differences between each category, it is a simplification for the logistic regression model and does not influence our interpretation of the result.
...
Model adjustments are made through polynomial transformation and the backward stepwise method. Polynomial transformation is to add the square form of nonlinear variables to the regression model, which can better capture the nonlinear and enhance the flexibility of the model. In addition to polynomial transformation, model improvement we also consider includes backward stepwise filtering methods. Backward stepwise regression starts with a model containing all potential independent variables. It iteratively removes the least significant variable, one at a time, based on a specified criterion, such as the highest p-value. This process continues until only variables that have a significant contribution to the model's predictive power remain.   
...
```{r}
table1 <- data %>%
  select(MISTMNT = mistmnt, MISPHLPF = misphlpf, MISHOPLES = mishopls, MISNOWRK = misnowrk, 
         Sex = sex, X_EDUCA = x_educag, X_INCOMG = x_incomg, VETERAN3 = veteran3, X_BMI5CAT = x_bmi5cat, Sleep_well = sleep_well) %>%
  tbl_summary(
    by = Sleep_well,
...
At the same time, based on table 1, all groups have relatively normal sample size, without extreme skew trend and small or large value in a single group. This ensures the analysis below are reliable, allowing using proportion in visualization part.
...
```{r}
# Set the margin and allow plotting outside the plot region
par(mar = c(5, 4, 4, 2) + 0.1, xpd = TRUE)

# Create the QQ plot
qqnorm(data$misnowrk, main = "", xlab = "", ylab = "Sample Quantiles", cex.main = 1, font.main = 1.5)
qqline(data$misnowrk, col = 2)

# Add caption at the bottom of the plot
mtext("Figure 2: QQ Plot of misnowrk. The Q-Q plot deviates from the line, suggesting that the variable misnowrk \ndoes not adhere to a normal distribution.", 
      side = 1, line = 4, adj = 0, font = 1.5, cex = 0.8)

# Move up the x-axis label
mtext("Theoretical Quantiles", side = 1, line = 2, adj = 0.5, font = 1, cex = 1.2)
...
These VIF values all fall between 1 and 5, suggesting that multicollinearity is not likely to be influencing the estimates of the coefficients significantly. This indicates that the regression model is based on independent variables that are not excessively linearly related, allowing for a more reliable interpretation of each variable's effect on sleep quality.
...
cbinded %>%
  kbl(caption = "Table 4: Binary Logistic Regression Analysis with 95% CI") %>%
  kable_classic(full_width = F, html_font = "Cambria")
```
...
hoslem_test_result <- hoslem.test(y = model$fitted.values, x = model$model$sleep_well_binary, g = 10)
...
# Extract degrees of freedom from the model summary
model_summary <- summary(model)
residual_dof <- model_summary$df.residual
null_dof <- model_summary$df.null

# Compile results into a data frame with formatted values
results_df <- data.frame(
  Statistic = c("Hosmer-Lemeshow Test Chi-Square", "Hosmer-Lemeshow Test P-Value", "AUC", 
                "Residual Deviance", "AIC", "Residual Degrees of Freedom", "Null Degrees of Freedom"),
  Value = format(round(c(hoslem_test_result$statistic, hoslem_test_result$p.value, auc_value, 
            residual_deviance, aic_value, residual_dof, null_dof), 2), scientific = FALSE)
)

# Convert the data frame to a kable format and apply styling
results_table <- kable(results_df, format = "html", booktabs = TRUE) %>%
...
### Key Findings from the Binary Logistic Regression Analysis
...
These results suggest that both mental health and sociodemographic factors play significant roles in sleep quality, but Mental Health Treatment variable is not significant and performs well in this logistic regression model, which is different from the result of Chi-square test.


## Conclusion

The analysis of the BRFSS 2013 dataset has discovered how various mental health and sociodemographic factors are associated with sleep quality among young adults and adults in the United States. The study's main findings indicate that factors such as feelings of hopelessness, education level, income, gender, and veteran status all play a significant role in determining sleep quality with directly linear association. Notably, higher levels of education and income are consistently associated with better sleep quality, while increased feelings of hopelessness and being a veteran are linked with poorer sleep outcomes. Gender differences also appear, with males reporting lower sleep quality than females.

The non-linear relationships observed for some variables, such as BMI and the perception of empathy impact sleep quality, highlighting the importance for people to keep these index within a suitable range. 

Interestingly, the mental health treatment did not show a significant association with sleep quality in the logistic regression model, contrasting with the results from the Chi-square test. This difference may be due to the complexity of the relationship between mental health care and sleep quality and underscores the importance of model choice in statistical analyses.

### Broader Implications
Our analysis highlights the need for a multifaceted perspective on sleep quality research. Such comprehensive insights are valuable for sleep management strategies that are more useful than single-variable or single-aspect models. It offers a potential improvement direction in both clinical practice and public health frameworks, like design different kinds of sleep illness treatment by patients mental status. For example, different treatment can be applied based on whether they are receiving mental illness treatment. These results aimed at enhancing sleep health by providing mental health support and mitigating adverse socioeconomic influences.

### Future Direction
Subsequent research should strive to delineate the causal relationships implicated by the current findings, thereby transcending the observational constraints of this study. Longitudinal designs would provide a more reliable dataset, coupled with a more expansive array of variables, could offer a deeper comprehension of the factors influencing sleep quality. 

While we have chosen to concentrate on a binary classification of sleep quality and got significant result, it is worth noting that it may be beneficial to further categorize the outcome variable into three classes, including less sleep time, good sleep time and more sleep time. This would be helpful for future research focusing on analyzing the factors contributing to long or short sleep durations, . 

### Limitations and Considerations
This study is not devoid of limitations. Selection bias that are from a modest response in this questionnaire and some unknown implications of encoding categorical variables ordinally must be acknowledged, as they may bear upon the findings' generalizability. These limitation causes a cautious interpretation of the results. Furthermore, although the sample size is large enough, the drop of all missing values and refused answers might limit the generalizability and relevance of the outcomes to a wider population. It is important for future studies to consider these limitations, thereby refining the understanding of the determinants of sleep quality.
...
Appendix 1: Observed and Expected Values for All Categorical Variables. 

```{r, echo=FALSE, results='asis'}
# Initialize two lists to store the kable formatted tables
observed_tables_list <- list()
expected_tables_list <- list()

# Function to convert a table to kable format and apply specified styling
convert_to_kable <- function(table) {
  kable_table <- kable(table, format = "html", booktabs = TRUE) %>%
                 kable_classic(full_width = TRUE, html_font = "Cambria")
  return(kable_table)
}

# Define your contingency tables and calculate expected values
tables_and_expected <- list(
  mistmnt = xtabs(~ sleep_well + mistmnt, data = data_combine),
  misphlpf = xtabs(~ sleep_well + misphlpf, data = data_combine),
  mishopls = xtabs(~ sleep_well + mishopls, data = data_combine),
  sex = xtabs(~ sleep_well + sex, data = data_combine),
  x_educag = xtabs(~ sleep_well + x_educag, data = data_combine),
  x_incomg = xtabs(~ sleep_well + x_incomg, data = data_combine),
  x_bmi5cat = xtabs(~ sleep_well + x_bmi5cat, data = data_combine),
  veteran3 = xtabs(~ sleep_well + veteran3, data = data_combine)
)

# Process each table
for (table_name in names(tables_and_expected)) {
  # Store the observed table in kable format
  observed_tables_list[[table_name]] <- convert_to_kable(tables_and_expected[[table_name]])

  # Calculate, round, and store the expected table in kable format
  expected_values <- round(chisq.test(tables_and_expected[[table_name]])$expected)
  expected_tables_list[[table_name]] <- convert_to_kable(as.table(expected_values))
}

# Variable descriptions
variable_descriptions <- list(
  mistmnt = "**MISTMNT**(Binary): Currently receiving mental health treatment or not.",
  misphlpf = "**MISPHLPF**(Ordinal): The level of recognition of others' empathy.",
  mishopls = "**MISHOPLES** (Ordinal): The degree of hopelessness experienced.",
  sex = "**Sex**(Binary): Female =1, Male =0.",
  x_educag = "**X_EDUCA**(Categorical): The level of the highest level of education.",
  x_incomg = "**X_INCOMG**(Categorical): The level of income.",
  x_bmi5cat = "**X_BMI5CAT**(Categorical): The category of BMI.",
  veteran3 = "**VETERAN3** (Binary): Have military experience or not."
)

# Print both observed and expected tables with descriptions
for (table_name in names(observed_tables_list)) {
  cat(variable_descriptions[[table_name]], "\n")
  cat("\nObserved Values:\n")
  print(observed_tables_list[[table_name]], row.names = FALSE)
  cat("\nExpected Values:\n")
  print(expected_tables_list[[table_name]], row.names = FALSE)
