...
```{r, message=FALSE, warning=FALSE, echo=FALSE}
# List to store the expected values
expected_values_list <- list()

# Function to calculate expected values and add them to the list
calculate_expected_values <- function(table) {
  expected_values <- chisq.test(table)$expected
...
tables_and_expected <- list(
  ...
  misphlp = xtabs(~ sleep_well + misphlpf, data = data_combine),
...
# Calculate and store expected values in the list
for (table_name in names(tables_and_expected)) {
  expected_values_list[[table_name]] <- calculate_expected_values(tables_and_expected[[table_name]])
}

# Print the list of expected values
