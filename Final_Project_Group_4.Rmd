...
write.csv(brfss_sleep_mental_omit, file = here("brfss_sleep_mental_omit.csv"))
```
```{r}
...
brfss_sleep_mental_adult <- brfss_sleep_mental_omit %>%
  ...
  mutate(sleep_well = case_when(
    ...
    sleptim1 < 7 ~ "insufficient",
    sleptim1 > 9 ~ "excessive",
...
brfss_sleep_mental_older_adults <- brfss_sleep_mental_omit %>% 
  filter(adult != 1) %>%
  mutate(sleep_well = case_when(
  sleptim1 >= 7 & sleptim1 <= 8 ~ "good",
  sleptim1 < 7 ~ "insufficient",
  sleptim1 > 8 ~ "excessive",
  TRUE ~ NA_character_ # for missing or other values
  ))
...
head(brfss_sleep_mental)
write.csv(brfss_sleep_mental, file = here("brfss_sleep_mental.csv"))
```
```{r}
# factors <- tolower(c("SLEPTIM1",  "X_AGE65YR", "MISNERVS", "MISHOPLS", "MISRSTLS", "MISDEPRD", 
#                "MISEFFRT", "MISWTLES", "MISNOWRK", "MISTMNT", "MISTRHLP", 
...
```{r}
# 箱线图比较男女睡眠时长
ggplot(brfss_sleep_mental, aes(x = factor(sex), y = sleptim1, fill = factor(sex))) +
  geom_boxplot() +
...
```{r}
data.mat <- xtabs(~ sleep_well + sex, data = brfss_sleep_mental)
data.mat
...
```
```{r}
data_mishopls <- xtabs(~ sleep_well + mishopls, data = brfss_sleep_mental)


data_mishopls

expected <- chisq.test(data_mishopls)$expected
expected
```
**None of the expected counts in this table is smaller than 5, so we should use X2 test statistic.**
```{r}
chisq.test(data_mishopls,correct=FALSE)$statistic %>% round(3)
chisq.test(data_mishopls,correct=FALSE)$p.value %>% round(3)
```
...
data_mistmnt <- matrix(c(336,840, 700, 4122), byrow=TRUE, nrow=2)
data_mistmnt

expected <- chisq.test(data_mistmnt)$expected
expected
```
**None of the expected counts in this table is smaller than 5, so we should use X2 test statistic.**
```{r}
chisq.test(data_mistmnt,correct=FALSE)$statistic %>% round(3)
chisq.test(data_mistmnt,correct=FALSE)$p.value %>% round(3)
```
...
```
```{r}
data_misphlpf <- xtabs(~ sleep_well + misphlpf, data = brfss_sleep_mental)
data_misphlpf

expected <- chisq.test(data_misphlpf)$expected
expected
```
**None of the expected counts in this table is smaller than 5, so we should use X2 test statistic.**
```{r}
chisq.test(data_misphlpf, correct=FALSE)$statistic %>% round(3)
chisq.test(data_misphlpf, correct=FALSE)$p.value %>% round(3)
```
